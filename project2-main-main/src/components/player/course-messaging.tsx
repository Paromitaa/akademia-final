...
export function CourseMessaging({ courseId }: { courseId: string }) {
  const { user } = useAuth();
  const { messages, loading, sending, sendMessage, scrollRef } = useCourseMessages(courseId);
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center gap-3 border-b border-border p-4">
        <Avatar className="h-10 w-10">
          ...
          <AvatarFallback>{course ? initials(course.instructor.name) : '?'}</AvatarFallback>
        </Avatar>
        <div className="min-w-0 flex-1">
          <p className="text-sm font-semibold text-foreground">{course?.instructor.name}</p>
          <p className="truncate text-xs text-muted-foreground">{course?.instructor.title}</p>
        </div>
