...
function defaultAvatar(role: UserRole): string {
  const avatars: Record<UserRole, string> = {
    student:
      'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=128&h=128&fit=crop&crop=faces&q=80',
    instructor:
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signIn = async (email: string, password: string, role?: UserRole): Promise<AuthUser> => {
    ...
    try {
      ...
      // Backend is unreachable — fall back to demo accounts
...
const DEMO_ACCOUNTS = [
  ...
  { email: 'admin@akademia.com', password: 'admin123', user: { id: 'u_admin', name: 'James Park', email: 'admin@akademia.com', role: 'admin' as UserRole, avatarUrl: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=128&h=128&fit=crop&crop=faces&q=80' } },
];
